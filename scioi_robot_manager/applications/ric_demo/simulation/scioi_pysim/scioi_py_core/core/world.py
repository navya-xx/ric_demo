...
import applications.ric_demo.simulation.scioi_pysim.scioi_py_core.core.spaces as core_spaces
import applications.ric_demo.simulation.scioi_pysim.scioi_py_core.core.scheduling as scheduling
import applications.ric_demo.simulation.scioi_pysim.scioi_py_core.core.physics as physics
...
class World(scheduling.ScheduledObject):
    ...
    def addObject(self, objects: Union[WorldObject, dict]):
        ...
        for obj in objects:
            ...
            # Check if the object already exists in  the list of objects. If so, just raise a warning and continue
            if obj in self.objects.values():
                print("Object already exists in world")
                continue

            for _, other in self.objects.items():
                if obj.name == other.name:
                    print(f"There already exists an object with name \"{obj.name}\".")
                    break
            ...
            print(f"Added Object \"{obj.name}\" {type(obj)} to the world.")

            # Add the methods to the scheduler
