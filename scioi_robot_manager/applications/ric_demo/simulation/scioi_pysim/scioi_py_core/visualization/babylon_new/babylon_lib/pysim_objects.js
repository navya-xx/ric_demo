...
class TWIPR_Robot extends WorldObject {
     ...
     // addTextToMesh(mesh, scene) {
     //    var dynamicTexture = new BABYLON.DynamicTexture("dynamicTexture", {width:512, height:512}, scene, true);
     //    dynamicTexture.hasAlpha = true;
     //    dynamicTexture.diffuseColor = new BABYLON.Color3(0.5,0.5,1)
     //    dynamicTexture.drawText("Hello, World!", 75, 240, "bold 44px Arial", "black", "transparent", true);
     //
     //    var material = new BABYLON.StandardMaterial("textMaterial", scene);
     //    // material.diffuseColor = new BABYLON.Color3(0.5,0.5,1)
     //    material.diffuseTexture = dynamicTexture;
     //    material.backFaceCulling = False // Show texture on both sides of the mesh
    ...
    update(sample) {
         if ('position' in sample){
             this.setPosition(sample['position'])
         }
         if ('orientation' in sample){
             this.setOrientation(sample['orientation'])
         }
        // if ('configuration' in sample) {
        //     if ('pos' in sample['configuration']){
        //         this.setPosition(sample['configuration']['pos'])
