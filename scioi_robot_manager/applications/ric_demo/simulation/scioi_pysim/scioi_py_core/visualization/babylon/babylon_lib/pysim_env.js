        ...
        if ('world' in sample){
            for (const [key, value] of Object.entries(sample['world']['objects'])){
                if (key in world_objects){
                    ...
                    console.log(value['parameters'].object_type)
                    let babylon_object_name
                    if (value['parameters'].object_type in this.config['object_config']){
                        babylon_object_name = this.config['object_config'][value['parameters'].object_type]['BabylonObject']
                        let objectPtr = eval(babylon_object_name)
                        world_objects[key] = new objectPtr(this.scene, key, value['parameters'].object_type, value['parameters'], this.config['object_config'][value['parameters'].object_type]['config'])
